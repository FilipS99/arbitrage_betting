...
def scrape_totolotek() -> Tuple[pd.DataFrame, list]:
    # totolotek 1 liga
    links = [
        ('https://www.totolotek.pl/pl/tenis', 'tennis', 'two-way'),
        ('https://www.totolotek.pl/pl/mma', 'ufc', 'two-way'),
        ('https://www.totolotek.pl/pl/pilka-nozna/polska',
         'polish football', 'three-way'),
        ('https://www.totolotek.pl/pl/pilka-nozna/finlandia',
         'finnish football', 'three-way'),
        ('https://www.totolotek.pl/pl/rugby', 'rugby', 'three-way'),
        ('https://www.totolotek.pl/pl/pilka-nozna/brazylia',
    ...
    for link in links:
        ...
        if url != driver.current_url:
            errors.append("Totolotek: URL Redirected to: " +
                          driver.current_url)
            continue
        ...
        while last_elements_count != current_elements_count:
            ...
            if current_elements_count > 0:
                ...
                driver.execute_script(
                    "arguments[0].scrollIntoView(false);", elements[-1])
            ...
            try:
                ...
                if len(item) > 4:
                    ...
                    if not re.match(date_pattern, game_datetime):
                        errors.append(
                            "Totolotek: Datetime error: " + game_datetime)
                        continue
                ...
                if bet_outcomes == 'two-way':
                    ...
                    if len(item) < 8 or not item[5].replace(".", "").isnumeric() or not item[7].replace(".", "").isnumeric():
                        errors.append(
                            "Totolotek: Error appending - " + " | ".join(item))
                        continue
